    ...
    public function extractStyle($html)
    {
        ...
        foreach ($match[1] as $code) {
            ...
            if (isset($tmp['type']) && strtolower($tmp['type']) === 'text/css' && isset($tmp['href'])) {
                ...
                // if "http://" in the url
                if (strpos($url, 'http://') !== false) {// get the domain "http://xxx/"
                    $url = str_replace('http://', '', $url);
                    $url = explode('/', $url);
                    $urlMain = 'http://'.$url[0].'/';// get the absolute url of the path
                    $urlSelf = $url;
                    unset($urlSelf[count($urlSelf)-1]);
                    $urlSelf = 'http://'.implode('/', $urlSelf).'/';// adapt the url in the css content
