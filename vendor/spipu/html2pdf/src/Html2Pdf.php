...
<html>
        ...
        if (!$this->_subPart && !$this->_isSubPart) {
            if (is_array($this->_background)) {
                ...
                if (isset($this->_background['img']) && is_array($this->_background['img'])) {
                    ...
                    $imageHeight = $imageWidth * $this->_background['img']['height'] / $this->_background['img']['width'];$posX = $this->cssConverter->convertToMM($this->_background['posX'], $this->pdf->getW() - $imageWidth);
                    $posY = $this->cssConverter->convertToMM($this->_background['posY'], $this->pdf->getH() - $imageHeight);$this->pdf->Image($this->_background['img']['file'], $posX, $posY, $imageWidth);
                }
            }
    ...
    protected function _calculateTableCellSize(&$cases, &$corr)
    {
        ...
        for ($x=0; $x<$amountCorr0; $x++) {
            for ($y=0; $y<$amountCorr; $y++) {
                if (isset($corr[$y][$x]) && is_array($corr[$y][$x]) && $corr[$y][$x][2]>1) {// sum the max width of each column in colspan
        ...
        for ($y=0; $y<$amountCorr; $y++) {
            for ($x=0; $x<$amountCorr0; $x++) {
                if (isset($corr[$y][$x]) && is_array($corr[$y][$x]) && $corr[$y][$x][3]>1) {// sum the max height of each line in rowspan
                    $s = 0;
            ...
            for ($x=0; $x<$amountCorr0; $x++) {
                if (isset($corr[$y][$x]) && is_array($corr[$y][$x])) {
                    ...
                    // with rowspan
                        ...
                        $cases[$corr[$y][$x][1]][$corr[$y][$x][0]]['h'] = $s;    for ($j=1; $j<$corr[$y][$x][3]; $j++) {
                            $tx = $x+1;
                            $ty = $y+$j;
        ...
        for ($tempPos = $this->_parsePos + 1; $tempPos<$amountHtmlCodes; $tempPos++) {
            ...
            if ($action->getName() === 'legend' && !$action->isClose()) {
                ...
                for ($sub->_parsePos = 0; $sub->_parsePos<$amountSubHtmlCodes; $sub->_parsePos++) {
                    $action = $sub->parsingHtml->code[$sub->_parsePos];
                    $sub->_executeAction($action);if ($action->getName() === 'legend' && $action->isClose()) {
                        break;
                    }
                }
        ...
        if ($this->_subPart) {
            ...
            for ($k=0; $k<$nbCases; $k++) {
                ...
                for ($i=0; $i<$nbCasesK; $i++) {
                    $h = max($h, self::$_tables[$param['num']]['cases'][$k][$i]['h']);if (self::$_tables[$param['num']]['cases'][$k][$i]['rowspan'] == 1) {
        ...
        if (!$this->_subPart) {
            ...
            if (!$this->_isInTfoot && self::$_tables[$param['num']]['td_y'] + self::$_tables[$param['num']]['marge']['b'] + $ty +$hfoot> $this->pdf->getH() - $this->pdf->getbMargin()) {
                ...
                if (count(self::$_tables[$param['num']]['tfoot']['code'])) {
                    ...
                    $tmpTD = self::$_tables[$param['num']]['td_curr'];
                    $oldParsePos = $this->_parsePos;
                    $oldParseCode = $this->parsingHtml->code;self::$_tables[$param['num']]['tr_curr'] = self::$_tables[$param['num']]['tfoot']['tr'][0];
                    self::$_tables[$param['num']]['td_curr'] = 0;
                    $this->_parsePos = 0;
                    $this->parsingHtml->code = self::$_tables[$param['num']]['tfoot']['code'];
                    $this->_isInTfoot = true;
                    $this->_makeHTMLcode();
                    $this->_isInTfoot = false;$this->_parsePos =     $oldParsePos;
                    $this->parsingHtml->code = $oldParseCode;
                    self::$_tables[$param['num']]['tr_curr'] = $tmpTR;
                    self::$_tables[$param['num']]['td_curr'] = $tmpTD;
                }
                ...
                if (isset(self::$_tables[$param['num']]['height'][self::$_tables[$param['num']]['page']])) {
                    ...
                    $this->parsingCss->value = self::$_tables[$param['num']]['style_value'];$this->_drawRectangle(
                        self::$_tables[$param['num']]['curr_x'],
                        self::$_tables[$param['num']]['curr_y'],
                        self::$_tables[$param['num']]['width'],
